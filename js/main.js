...
$(function () {
    const container = document.getElementById("partner_logo_list");
    for(let i = 0; i < 2; i++) {
       partnerLogos.forEach((logo) => {
        const img = document.createElement("img");
        img.src = partnerLogoBasePath + logo.fileName;
        img.alt = logo.alt;
        img.classList.add('logo_ticker_image');
...
$(function () {
  productList.map((product) => {
    $("#product-items--container").append(`
      <div data-filterable data-filter-category="${product.category}" class="relative col-span-3 overflow-hidden group hover:shadow-md ">
